...
export const TaskManager: React.FC = () => {
    ...
    const [taskTabs, setTaskTabs] = useState<Record<string, 'result' | 'curl'>>({});
    ...
    const pollTask = async (taskId: string) => {
        ...
        try {
            ...
            // Set default tab for new task
            setTaskTabs(prev => ({ ...prev, [taskId]: prev[taskId] || 'result' }));
        } catch (e: any) {
    ...
    return (
        <div className="p-4 bg-terminal-surface rounded-lg shadow-sm h-full flex flex-col">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold text-terminal-text">Tasks Utility</h2>
                <Button
                    variant="outline"
                    size="sm"
                    onClick={loadTasks}
            ...
            {error && (
                <div className="bg-terminal-red/10 border border-terminal-red text-terminal-red p-3 rounded mb-4 text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-full">
                ...
                <div className="bg-terminal-bg rounded p-4 border border-terminal-text-muted overflow-auto">
                    <h3 className="font-medium text-terminal-text mb-3 flex items-center gap-2">
                        <ClipboardList className="w-4 h-4" /> Available Tasks
                    </h3>
                    {tasks.length === 0 ? (
                        <p className="text-sm text-terminal-text-muted">No tasks found or not supported.</p>
                    ) : (
                        <div className="space-y-2">
                            {tasks.map((task, idx) => (
                                <div key={idx} className="p-3 border border-terminal-border rounded bg-terminal-surface/20">
                                    <div className="font-medium text-terminal-text">{task.name}</div>
                                    <div className="text-xs text-terminal-text-muted mt-1">{task.description}</div>
                                </div>
                            ))}
                        </div>
                    )}
                ...
                <div className="bg-terminal-bg rounded p-4 border border-terminal-text-muted overflow-auto">
                    ...
                    <div className="flex gap-2 mb-6">
                        <input
                            ...
                            className="flex-1 rounded-md border border-terminal-text-muted bg-terminal-bg px-3 py-2 text-sm text-terminal-text"
                            id="manual-task-id"
                        />
                        <Button
                            size="sm"
                    ...
                    <div className="space-y-4">
                        {executions.map(exec => (
                            <div key={exec.id} className="p-3 border border-terminal-border rounded bg-terminal-surface/10">
                                <div className="flex justify-between items-start mb-2">
                                    <span className="font-mono text-xs font-bold text-terminal-text">{exec.id}</span>
                                ...
                                <div className="space-y-3 mt-3">
                                    <div className="flex gap-4 border-b border-zinc-700/30">
                                        <button
                                            onClick={() => setTaskTabs(prev => ({ ...prev, [exec.id]: 'result' }))}
                                            className={`pb-1 text-[10px] font-bold transition-all ${taskTabs[exec.id] === 'curl' ? 'text-terminal-text-muted hover:text-terminal-text' : 'border-b-2 border-terminal-cyan text-terminal-cyan'}`}
                                        >
                                            RESULT
                                        </button>
                                        <button
                                            onClick={() => setTaskTabs(prev => ({ ...prev, [exec.id]: 'curl' }))}
                                            className={`pb-1 text-[10px] font-bold transition-all ${taskTabs[exec.id] === 'curl' ? 'border-b-2 border-terminal-cyan text-terminal-cyan' : 'text-terminal-text-muted hover:text-terminal-text'}`}
                                        >
                                            CURL
                                        </button>
                                    </div>
                                    ...
                                    <div className="mt-2 min-h-[60px]">
                                        {taskTabs[exec.id] !== 'curl' ? (
                                            <pre className="text-xs text-terminal-text-muted overflow-auto max-h-[150px] bg-terminal-surface/50 p-3 rounded border border-zinc-700/30 font-mono">
                                                {exec.result || 'No result data.'}
                                            </pre>
                                        ) : (
                                            exec.curl ? (
                                                <CurlPreview command={exec.curl} title="cURL" allowToggle={false} />
                                            ) : (
                                                <div className="text-[10px] text-terminal-text-muted italic p-2 border border-dashed border-zinc-700/30 rounded">
                                ...
                                <div className="mt-3 flex justify-end">
                                    <Button size="icon" variant="ghost" onClick={() => pollTask(exec.id)} title="Update status" className="h-8 w-8">
