    ...
    return (
        <div className="p-4 bg-terminal-surface rounded-lg shadow-sm">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold text-terminal-text">Resources</h2>
            ...
            {loading && <p className="text-terminal-text-muted text-sm">Loading resources...</p>}
            ...
            {!loading && !error && resources.length === 0 && (
                <p className="text-terminal-text-muted text-sm">No resources found.</p>
            )}

            <div className="space-y-4">
                {resources.map((resource: any) => (
                    <div key={resource.uri} className="p-3 bg-terminal-bg rounded-md border border-terminal-border text-sm hover:border-terminal-text-muted transition-colors">
                        <div className="flex justify-between items-start">
                            <span className="font-medium truncate mr-2 text-terminal-text">{resource.name}</span>
                            <span className="text-xs px-2 py-0.5 rounded-full bg-terminal-cyan/20 text-terminal-cyan font-medium">
                                {resource.mimeType || 'unknown'}
                            </span>
                        </div>
                        <div className="text-xs text-terminal-text-muted mt-1 font-mono break-all">{resource.uri}</div>
                        {resource.description && (
                            <p className="text-terminal-text-muted mt-1">{resource.description}</p>
                        )}
                        ...
                        {resource.content && (
                            <div className="mt-2 p-2 bg-terminal-surface rounded border border-terminal-text-muted">
                                <pre className="text-xs whitespace-pre-wrap font-mono text-terminal-text max-h-[200px] overflow-auto">
                                    {resource.content}
                                </pre>
                            </div>
                        )}
